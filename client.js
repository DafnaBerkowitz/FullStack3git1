  ...
  if (book.ariaHidden = true) {
    ...
    function validate_book() {
      if (fxhttp.status == '404') {
        ...
        let boo1 = response.book[0] ? response.book[0].name : ' ';
...
async function picLink2(nameBook, id, id2) {
  //books1();
  document.getElementById(id).className = "visible";
  document.getElementById(id).innerHTML = "השאלת ספר";
  document.getElementById(id2).className = "hide";
  let name = document.cookies;
  name = name.split(/[=,]/);
  ...
  function validate_book() {
    if (fxhttp.status == '404') {
      ...
      let response = JSON.parse(fxhttp.responseText);
      let boo1 = response.book[0] !== undefined ? response.book[0].name : ' ';
      let boo2 = (response.book[1] ? response.book[1].name : ' ');
      let boo3 = (response.book[2] ? response.book[2].name : ' ');
      document.cookies = "currenUser=" + name + ",book1=" + boo1 + ",book2=" + boo2 + ",book3=" + boo3;
    }
...
function openForm() {
  ...
  document.getElementById('book1').innerHTML = "Book1: " + (name[3]!== undefined ? name[3] : ' ');
  document.getElementById('book2').innerHTML = "Book2: " + (name[5]!== undefined ? name[5] : ' ');
  document.getElementById('book3').innerHTML = "Book3: " + (name[7] !== undefined ? name[7] : ' ');
